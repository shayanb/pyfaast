...
import pyfaast.api as api
from pyfaast.errors import InitializationError, RecsTimeout
from pyfaast.models import Profile, User, RateLimited, Match, Friend


class Session(object):

    def __init__(self, pair = None, facebook_token=None, XAuthToken=None, proxies=None, facebook_id=None):
        if facebook_token is None and XAuthToken is None:
            raise InitializationError("Either XAuth or facebook token must be set")

        self._api = api.Faast(XAuthToken, proxies)
